...
```json
{
  ...
  "properties": {
    "path": {
      "type": "string",
      "pattern": "^(chrysalis/Tasks|Projects|Slipbox|Sources)/.*\\.md$",
...
1. **Agent Generates `PlanProposal`**:
   ...
   ```json
   {
     ...
     "mutations": [
       ...
       {
         "action": "create_record",
         "path": "chrysalis/Tasks/cs410-hw1.md",
         "type": "task",
...
| `path_traversal_forbidden` | mdbase v0.3 Core | `error` | `FixRequest` | Path contains `..` or escapes the collection root directory. | Confine path to collection subdirectories (`chrysalis/Tasks/`, `Projects/`, etc.). |
