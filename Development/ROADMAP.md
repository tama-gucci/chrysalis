...
3. **Execution Layer:** Universal Chrysalis schemas (`chrysalis/Tasks/*.md`) with deep Obsidian interoperability.
...
- **Option A (Dedicated Home Hub - Golem):** Surface Pro X (16GB RAM total: 4GB Hyper-V Home Assistant, 12GB operational memory for Chrysalis and background agents) hosting the Ambient Gateway daemon (`apps/gateway/`) on port **8765** behind a Cloudflare Zero-Trust Tunnel, coexisting with chrysalis-obsidian on port **8080**.
...
- **Prerequisites & Technical Dependencies**:
  - `chrysalis/_templates/Task-Template.md` schema compliance (`micro_chunked`, `googleCalendarEventId`).
...
- **Capability**: 
  ...
  - Configure the 24/7 Ambient Gateway daemon on Golem on port **8765** (reserving port **8080** for chrysalis-obsidian) exposed securely via Cloudflare Zero-Trust Tunnel.
...
- **Capability**: 
  ...
  - Verify complete coexistence and synchronization between chrysalis-obsidian local server on port **8080** and Chrysalis Ambient Gateway on port **8765**.
- **Prerequisites & Technical Dependencies**:
...
- **Capability**: Automated bidirectional semantic linking between active project roadmaps (`Projects/*/Roadmap.md`), permanent slipbox literature notes (`Slipbox/`), and universal task notes (`chrysalis/Tasks/`), revealing hidden dependencies and knowledge clusters.
- **Prerequisites & Technical Dependencies**:
...
| **Near-term (Sprint 1)** | Task Micro-chunking | Active | M1 | `chrysalis/` | Schema validation, Starter Wedge templates | Zero PII risk |
...
| **Mid-term** | Obsidian Port 8080 Coexistence | Active | M4 | Obsidian / chrysalis-obsidian | Port separation verification | Localhost only |
